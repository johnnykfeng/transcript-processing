...
def transcript_token_size(raw_transcript, verbose=True):
  ...
  if verbose:
      ...
      print(
          f'Cost of input prompt with gpt-3.5-turbo: ${token_len*0.002/1000}')
...
```{transcript}```"""
  ...
  result = chat(chat_prompt.format_prompt(
      transcript=transcript).to_messages())
...
```{text}```"""
  ...
  result = chat(chat_prompt.format_prompt(
      text=text, user_prompt=user_prompt).to_messages())
  return result.content
